...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  register: async (data: RegisterDto) => {
    ...
    try {
      ...
      if (isEmailVerificationPending(response)) {
        // Account created — waiting for email OTP (one-time verification)
        set({
  ...
  verifyEmailOtp: async (code: string) => {
    ...
    if (!pendingVerificationEmail) {
      set({ error: 'Session expirée. Veuillez vous réinscrire.', emailVerificationPending: false })
      return
    }
    set({ isLoading: true, error: null })
    try {
      const response = await flowguardApi.verifyEmail(pendingVerificationEmail, code)
      await Keychain.setGenericPassword(
        'fg',
        JSON.stringify({ accessToken: response.accessToken, refreshToken: response.refreshToken }),
      )
